...
class TransformerWrapper(pl.LightningModule):
    ...
    def generate(
        ...
        save_midi=False,
        save_mix=False,
        save_rendered_midi=False,
        midi_path=None,
        mix_path=None,
        rendered_midi_path=None,
        click_amp=0.2,
        add_click=False,
        max_batch_size=None,
        beatsteps=None,
        ...
        if composer is None:
            composer = random.sample(
                list(composer_to_feature_token.keys()), 1)[0]
        ...
        if beatsteps is None:
            ...
            beatsteps = interpolate_beat_times(
                beat_times, steps_per_beat, extend=True)
        else:
            y = None
        ...
        if self.use_mel:
            if audio_y is None and config.dataset.sample_rate != ESSENTIA_SAMPLERATE:
                if y is not None:
                    ...
                    y, sr = librosa.load(
                        audio_path, sr=config.dataset.sample_rate)
        ...
        if show_plot or save_mix or save_rendered_midi:
            if mix_sample_rate != sr:
                y = librosa.core.resample(
                    y, orig_sr=sr, target_sr=mix_sample_rate)
                sr = mix_sample_rate
            if add_click:
                clicks = (
                    librosa.clicks(times=beatsteps, sr=sr,
                                   length=len(y)) * click_amp
                )
                y = y + clicks
        ...
        if save_rendered_midi:
            pm_y.write(
                data=pm_y,
                format="wav",
                samplerate=sr,
                file=rendered_midi_path,
            )
